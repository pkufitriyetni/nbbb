...
local isBringingItems = false

-- ========== GLOBAL VARIABLES FOR ITEM SELECTION ========== (TAMBAHKAN INI)
_G.selectedItemName = nil
_G.selectedItemButton = nil
...
local HitboxSystem = {
  ...
  targets = {"Bunny", "Wolf", "Alpha Wolf", "Bear", "Pola Bear", "Cultish", "Alien", "Artic Fox"}
}
...
local ESP = {
  ...
  Targets = {
    animals = {"Bunny", "Wolf", "Alpha Wolf", "Bear", "Pola Bear", "Artic Fox", "Mammoth"},
    npcs = {"Cultish", "Alien", "Pelt trader", "Furniture Trader", "Fairy", "LostChild", "LostChild2", "LostChild3", "LostChild4"},
...
for _, obj in pairs(charactersFolder:GetChildren()) do
  if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") then
    ...
    -- Special matching for Cultish and Alien (any variation)
    if targetName == "Cultish" then
      isMatch = objName:lower():find("cultish") or objName:lower():find("cultist")
...
if item:IsA("Model") then
  -- Check by name
  if item.Name == "Old Axe" or item.Name == "Good Axe" or item.Name == "Strong Axe" then
    return true, item
  end
...
-- Function untuk mencari item dengan metode yang sama seperti Bring Item
local function findItemForTeleport(itemName)
local items = findItemBetter(itemName) -- Menggunakan function yang sudah ada
return items
end
...
"UFO Component", "UFO Scrap", "UFO Junk", "Cultist Experiment", "Cultist Prototype"
}
...
-- ========== SMART ITEM BRINGER FUNCTIONS (PENGGANTI YANG LAMA) ==========

-- Function untuk deteksi otomatis apakah item bisa di-interact
local function canInteractWithItem(item)
...
print("🔍 Analyzing item:", itemName)

-- ========== POSITIVE INDICATORS (Item BISA di-interact) ==========

-- Check 1: ProximityPrompt (indicator terkuat)
local proximityPrompt = item:FindFirstChildOfClass("ProximityPrompt", true)
if proximityPrompt and proximityPrompt.Enabled then
score = score + 10
table.insert(reasons, "✅ Has active ProximityPrompt")
print("  ✅ Found ProximityPrompt:", proximityPrompt.ActionText or "Pickup")
end

-- Check 2: ClickDetector
local clickDetector = item:FindFirstChildOfClass("ClickDetector", true)
if clickDetector then
score = score + 8
table.insert(reasons, "✅ Has ClickDetector")
print("  ✅ Found ClickDetector")
end

-- Check 3: Tool class (hampir pasti bisa di-pickup)
if item:IsA("Tool") then
score = score + 9
table.insert(reasons, "✅ Is a Tool")
print("  ✅ Item is a Tool")
end

-- Check 4: Handle part (biasanya item pickup punya Handle)
local handle = item:FindFirstChild("Handle")
if handle and handle:IsA("BasePart") then
score = score + 6
table.insert(reasons, "✅ Has Handle part")
print("  ✅ Found Handle part")
end

-- Check 5: Script yang handle pickup (RemoteEvent calls)
local hasPickupScript = false
for _, descendant in pairs(item:GetDescendants()) do
if descendant:IsA("LocalScript") or descendant:IsA("Script") then
  hasPickupScript = true
  break
end
end
if hasPickupScript then
score = score + 3
table.insert(reasons, "✅ Has scripts (possibly pickup)")
print("  ✅ Found scripts")
end

-- Check 6: Attribute "Pickupable" atau sejenisnya
for attributeName, attributeValue in pairs(item:GetAttributes()) do
if attributeName:lower():find("pickup") or
attributeName:lower():find("interact") or
attributeName:lower():find("collect") then
  score = score + 5
  table.insert(reasons, "✅ Has pickup attribute: " .. attributeName)
  print("  ✅ Found pickup attribute:", attributeName)
end
end

-- Check 7: Part yang tidak anchored (biasanya item pickup bisa di-move)
...
score = score + 2
table.insert(reasons, "✅ Has movable parts")
print("  ✅ Has non-anchored parts")
end

-- ========== NEGATIVE INDICATORS (Item TIDAK bisa di-interact) ==========

-- Check 8: Model dengan banyak parts (kemungkinan structure/building)
local partCount = 0
...
if partCount > 5 then
score = score - 3
table.insert(reasons, "❌ Too many parts (" .. partCount .. ") - likely structure")
print("  ❌ Too many parts:", partCount, "- might be structure")
end

-- Check 9: Semua parts anchored (kemungkinan building/static object)
...
if allAnchored and totalParts > 1 then
score = score - 4
table.insert(reasons, "❌ All parts anchored - likely static")
print("  ❌ All parts are anchored - likely static object")
end

-- Check 10: Size terlalu besar (kemungkinan building)
if item:IsA("Model") then
...
if success and size then
  ...
  if maxDimension > 20 then -- Lebih dari 20 studs
    score = score - 5
    table.insert(reasons, "❌ Too large (" .. math.floor(maxDimension) .. " studs)")
    print("  ❌ Item too large:", math.floor(maxDimension), "studs")
  end
end
end

-- ========== DECISION LOGIC ==========
local canInteract = score >= 5 -- Threshold: score minimal 5 untuk dianggap interactable

print("  📊 Final Score:", score, "| Can Interact:", canInteract)
if #reasons > 0 then
print("  📝 Reasons:")
...
return canInteract, score, reasons
end

-- Function untuk cari item dengan nama tertentu yang bisa di-interact (PENGGANTI findItemBetter)
local function findInteractableItems(targetItemName)
local foundItems = {}

print("🔎 Searching for interactable:", targetItemName)

-- Search in Workspace.Items (sesuai dengan scanner yang sudah ada)
local itemsFolder = Workspace:FindFirstChild("Items")
if not itemsFolder then
...
-- Function untuk search dalam folder (recursive)
...
for _, item in pairs(folder:GetChildren()) do
  ...
  if item.Name == targetItemName then
    local canInteract, score, reasons = canInteractWithItem(item)
    ...
    if canInteract then
      ...
      print("✅ Found interactable " .. targetItemName .. " with score " .. score)
    else
      print("❌ Found " .. targetItemName .. " but not interactable (score: " .. score .. ")")
    end
  end

  -- Search in subfolders
  if item:IsA("Folder") or item:IsA("Model") then
    searchInFolder(item, depth + 1)
  end
end
end

searchInFolder(itemsFolder, 0)

-- Sort by score (highest first)
table.sort(foundItems, function(a, b) return a.score > b.score end)

print("🎯 Found " .. #foundItems .. " interactable " .. targetItemName .. " items")
return foundItems
end

-- Function untuk bring item ke player (PENGGANTI bringItem)
local function bringItemToPlayer(itemData)
...
local bringPos = playerPos + Vector3.new(0, 3, 3) -- Slightly in front and above

print("📦 Bringing " .. item.Name .. " to player...")

-- Method 1: If it's a tool, try to equip directly
if item:IsA("Tool") then
pcall(function()
...
-- Method 2: Move to player position
pcall(function()
if item:IsA("Model") and item.PrimaryPart then
item:SetPrimaryPartCFrame(CFrame.new(bringPos))
elseif item:IsA("Model") then
item:MoveTo(bringPos)
elseif item:IsA("BasePart") then
item.CFrame = CFrame.new(bringPos)
end
print("  ✅ Item moved to position")
end)

-- Method 3: Enable interaction properties
pcall(function()
for _, part in pairs(item:GetDescendants()) do
if part:IsA("BasePart") then
  part.CanCollide = true
...
print("  ✅ Interaction properties enabled")
end)

return true
end

-- FUNCTION UTAMA: Bring all items dengan nama tertentu (PENGGANTI bringAllItems)
function bringAllItems(itemName)
showNotification("Searching for " .. itemName .. "...", 2)

local foundItems = findInteractableItems(itemName)
local totalFound = #foundItems

print("🔍 Found", totalFound, itemName)

if totalFound == 0 then
showNotification("No interactable " .. itemName .. " found!", 2)
return false
end

showNotification("Bringing " .. totalFound .. " " .. itemName .. "...", 2)

local successCount = 0

-- Bring ALL items (tidak ada limit max 5 lagi)
for i, itemData in ipairs(foundItems) do
if not isBringingItems then
  break
end

if bringItemToPlayer(itemData) then
  successCount = successCount + 1
end

wait(0.1) -- Small delay between items
end

showNotification("Brought " .. successCount .. "/" .. totalFound .. " " .. itemName, 2)
...
createToggleButton(leftFrame, "Kill Aura", UDim2.new(0, 0, 0.05, 0), UDim2.new(1, 0, 0.05, 0), toggleKillAura)
createSlider(
leftFrame,
"Kill Aura Radius",
UDim2.new(0, 0, 0, 0),
...
-- Fungsi Auto Fuel
local function toggleAutoFuel(enabled)
features.autoFuel = enabled
if enabled then
  showNotification("Auto Fuel enabled!", 2)
else
  showNotification("Auto Fuel disabled!", 2)
end
end
...
createToggleButton(rightFrame, "Auto Fuel", UDim2.new(0, 0, 0.05, 0), UDim2.new(1, 0, 0.05, 0), toggleAutoFuel)
createToggleButton(rightFrame, "God Mode", UDim2.new(0, 0, 0.05, 0), UDim2.new(1, 0, 0.05, 0), toggleGodMode)
createToggleButton(
rightFrame,
"Infinite Hunger",
UDim2.new(0, 0, 0.05, 0),
UDim2.new(1, 0, 0.05, 0),
toggleInfiniteHunger
)
...
-- BUAT BRING ITEMS FRAME (FULL WIDTH DI KANAN) - UKURAN NORMAL TANPA PERUBAHAN
local bringItemsFrame = Instance.new("Frame")
bringItemsFrame.Name = "BringItemsFrame"
bringItemsFrame.Size = UDim2.new(1, 0, 0, 300) -- UKURAN NORMAL FULL WIDTH
bringItemsFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
bringItemsFrame.BorderSizePixel = 0
bringItemsFrame.Parent = rightFrame
bringItemsFrame.LayoutOrder = 2

local bringFrameCorner = Instance.new("UICorner")
bringFrameCorner.CornerRadius = UDim.new(0, 8)
bringFrameCorner.Parent = bringItemsFrame

-- Title untuk bring items
local bringTitleLabel = Instance.new("TextLabel")
bringTitleLabel.Name = "BringTitleLabel"
bringTitleLabel.Size = UDim2.new(1, 0, 0, 30)
bringTitleLabel.Position = UDim2.new(0, 0, 0, 5)
bringTitleLabel.BackgroundTransparency = 1
bringTitleLabel.Text = "Item Bringer"
bringTitleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
bringTitleLabel.Font = Enum.Font.GothamBold
bringTitleLabel.TextSize = 16
bringTitleLabel.TextXAlignment = Enum.TextXAlignment.Center
bringTitleLabel.Parent = bringItemsFrame

-- Item categories (improved layout)
local categoriesFrame = Instance.new("Frame")
categoriesFrame.Name = "CategoriesFrame"
categoriesFrame.Size = UDim2.new(1, -10, 1, -70) -- Space for title and buttons
categoriesFrame.Position = UDim2.new(0, 5, 0, 35)
categoriesFrame.BackgroundTransparency = 1
categoriesFrame.Parent = bringItemsFrame
...
-- Helper function untuk membuat clickable listbox
local function createClickableListbox(parent, title, position, size, items, callback)
local frame = Instance.new("Frame")
frame.Name = title .. "Listbox"
frame.Parent = parent
frame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
frame.BorderSizePixel = 0
frame.Position = position
...
for i, item in ipairs(items) do
  ...
  showNotification("Selected: " .. item, 1)
...
-- Food Items
local foodItems = {
"Berry", "Carrot", "Cooked Morsel", "Cooked Steak", "Morsel", "Steak",
"Apple", "Cake", "Ribs", "Chili", "Hearty Stew", "Meat Sandwich", "Bandage", "MedKit"
}
createClickableListbox(
categoriesFrame,
"Food",
UDim2.new(0, 0, 0, 0),
UDim2.new(0.48, 0, 1, 0), -- UKURAN DISESUAIKAN UNTUK 2 KATEGORI
foodItems,
showNotification
)
...
-- TAMBAHKAN FRAME KEDUA UNTUK RESOURCES (BARIS KEDUA)
local categoriesFrame2 = Instance.new("Frame")
categoriesFrame2.Name = "CategoriesFrame2"
categoriesFrame2.Size = UDim2.new(1, -10, 0.4, 0) -- SEPARUH TINGGI
categoriesFrame2.Position = UDim2.new(0, 5, 0.6, -35) -- POSISI DI BAWAH
categoriesFrame2.BackgroundTransparency = 1
categoriesFrame2.Parent = bringItemsFrame

createClickableListbox(
categoriesFrame2,
"Resources",
UDim2.new(0, 0, 0, 0),
UDim2.new(1, 0, 1, 0), -- FULL WIDTH
resourceItems,
showNotification
)

-- Button Frame untuk BRING dan STOP (di bawah categories)
local buttonFrame = Instance.new("Frame")
buttonFrame.Name = "ButtonFrame"
buttonFrame.Size = UDim2.new(1, -10, 0, 35)
buttonFrame.Position = UDim2.new(0, 5, 1, -40)
buttonFrame.BackgroundTransparency = 1
buttonFrame.Parent = bringItemsFrame
...
-- BRING Button (improved design)
local bringButton = Instance.new("TextButton")
bringButton.Name = "BringButton"
bringButton.Size = UDim2.new(0.45, 0, 1, 0)
bringButton.BackgroundColor3 = Color3.fromRGB(0, 150, 0)
bringButton.Text = "BRING ITEM"
...
showNotification("Bringing " .. itemName .. "...", 2)
...
showNotification(itemName .. " brought successfully!", 2)
else
if _G.selectedItemButton then
...
showNotification("Failed to bring " .. itemName, 2)
end
